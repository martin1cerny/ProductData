...
namespace Example_7_Dimensions
{
    class Program
    {
        ...
        private T New<T>(Action<T> init) where T : IInstantiableEntity
        {
            return _model.Instances.New<T>(init);
        }

        private XbimEditorCredentials GetCredentials()
        {
            return new XbimEditorCredentials
            {
                ...
                EditorsOrganisationName = "UNMZ"
            };
        }

        private string GetPathInSamples(string directory, string name)
        {
        ...
        public void DynamicValues()
        {
            ...
            //using (var model = new MemoryModel(new Xbim.Ifc4.EntityFactoryIfc4()))
            {
                ...
                if (!validator.Check(model))
                    throw new Exception("There are errors in the model");

                var fileName = GetPathInSamples("DynamicProperties", "properties");
                model.SaveAs(fileName + ".ifcxml");
                model.SaveAs(fileName + ".ifc");
            }
        }
        ...
        public void ComplexProperty()
        {
            using (var model = CreateModel())
            {
                ...
                var outName = GetPathInSamples("SmartCEExamples", "Example_7_Dimensions");
                model.SaveAs(outName + ".ifc");
                var w = new XbimXmlWriter4(XbimXmlSettings.IFC4Add2);
                var xml = XmlWriter.Create(outName + ".ifcxml", new XmlWriterSettings { Indent = true, IndentChars = "  " });
